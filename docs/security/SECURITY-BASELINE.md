...
| 4 ⚠️ | 🟠 **High** | `editorIds`/`ownerId` auto-escribibles y sin validar → editor se convierte en owner / acuña co-admins | EoP | ✅ P1-b |
| 5 | 🟠 **High** | Sin capa anti-automatización: no App Check + `trackInteraction` público sin throttle (cost/availability-DoS) | DoS | P1 |
| 6 | 🟡 **Medium** | Sin validación de forma/tipo/tamaño en reglas — todo `*_MAX` es solo UI, evadible | Tampering | ✅ P1-b |
| 7 | 🟡 **Medium** | Escrituras a Storage de assets demasiado amplias (cualquier signed-in escribe en cualquier ruta) + sin límite de tamaño/tipo | Tampering/DoS | ✅ P1-c |
| 8 | 🟡 **Medium** (raíz↑) | Sin tests de reglas y sin CI — toda la frontera puede regresar en silencio (auto-deploy de `main`) | Assurance | P1 |
| 9 | 🟡 **Medium** | Headers de seguridad ausentes (CSP, HSTS, frame-ancestors, nosniff…) | Misconfig | ✅ P1-f |
| 10 | 🟡 **Medium** | Proofs (PII financiera) retenidos indefinidamente; sin policy/consentimiento/erasure | Privacy | P1/P2 |
| 11 ⚠️ | 🟡 **Medium** | Confirm/expire sin transacción ni state-machine guard + triggers sin `event.id` idempotency → doble-conteo | Integridad/race | ✅ P1-h |
| 12 ⚠️ | 🟡 **Medium** | Admin global único, sin scoping/2FA; acciones de admin (verify/delete) **no** se auditan | IR / blast radius | P2 |
| 13 ⚠️ | 🟡 **Medium** | Scraping del catálogo libre = exfiltración del grafo completo SMB+escuelas+contacto (riesgo de negocio) | Info Disclosure | 🟢 #13 (email→privado ✅; resto ops) |
| 14 | 🟡 **Medium** | Functions sin `maxInstances`/quota (default Gen2 ~1000) = denial-of-wallet | DoS/costo | P1 |
| 15 | 🟢 **Low** | `quoteUrls`/hrefs UGC sin validar esquema (`javascript:`/`data:`) | XSS | ✅ #15 |
...
| Editor reescribe `ownerId` a sí mismo / acuña editores | EoP | Editor de página | Control de la página + key anti-self-dealing | **P1-b:** `keepsOwner()`+`keepsEditors()` congelan ambos en updates no-admin → **mitigado** | ✅ |
| Owner spoofea denormalizados / sube payload gigante | Tampering | Owner / script | Integridad del catálogo + costo de render | **P1-b:** write-shape (`hasOnly`+type+caps+enum) por colección; computados ya congelados → **mitigado** | ✅ |
| Signed-in sobrescribe assets de competidor / sube archivos | Tampering/DoS | Cualquier usuario Google | Assets de marca + costo Storage | **P1-c:** assets gated por owner/editor del doc padre (`firestore.get`) + cap 12 MiB + matcher `contentType` en todo write → **mitigado** | ✅ |
...
| Clickjacking / blast-radius de futuro XSS sobre SSR+login | Tampering/EoP | Atacante con iframe / sink futuro | Sesión del owner logueado | **P1-f:** `frame-ancestors 'none'`+`X-Frame-Options: DENY` enforced (clickjacking); HSTS/nosniff/Referrer/COOP/Permissions enforced; CSP estricta en Report-Only (pendiente enforce) | ✅ |
...
- ✅ **P1-b** · **Validación de forma en reglas** — IMPLEMENTADO. Write-shape por colección (businesses, schools, schools/{id}/projects, subscriptions, projectContributions, donorProfiles): en **CREATE** `keys().hasOnly([...])` fija el set exacto que escribe cada write path (rechaza claves basura) + type guards + caps de string (`description<=300`, `displayName<=60`, `projectTitle<=120`) + enums (`currency`, `type`, `status`) + bounds numéricos (`units<=1000`, `amount==units*5000`); fuerza `status:'draft'` (business) / `'pending'` (school) / `'active'` (project). En **UPDATE** se valida solo lo cambiado con `diff().affectedKeys().hasOnly([...])` + caps guardados por `hasAny([campo])` — la regla de oro que evita romper writes parciales (gallery `arrayUnion`, status-only, proof, confirm): un `keys().hasOnly()` en update rechazaría el 100% de los updates legítimos porque el doc fusionado conserva los campos computados/inmutables. **#4 (HIGH):** `keepsOwner()` congela `ownerId` y `keepsEditors()` congela `editorIds` en el brazo no-admin (ningún write path de cliente los toca; cierra la toma de control editor→owner). El create de donación personal/aporte OMITE `units`/`amount`/`donorName` del doc público (defiende P0-d). Constantes `*_MAX` hardcodeadas (las reglas no importan TS). Harness +15 tests (allow de cada writer legítimo PRIMERO, luego deny de oversize/junk/extra/ownerId-change/editorIds-abuse/magnitud-pública): **77 firestore + 15 storage = 92 verdes**. → [`firestore.rules`](../../firestore.rules), [`test/rules/firestore.rules.test.ts`](../../test/rules/firestore.rules.test.ts). *(cierra #6 y #4)*
- ✅ **P1-c** · **Gate de escrituras de assets en Storage** — IMPLEMENTADO. `businesses/{id}/**` y `schools/{id}/**` pasaron de `write: if isSignedIn()` (cualquiera sobrescribía cualquier logo/cover) a **gated por owner/editor del doc padre** vía `firestore.get` (helpers `ownsBusinessId`/`ownsSchoolId`, espejando el patrón de los proofs). Todo `create/update` valida además **forma**: cap de tamaño `request.resource.size < 12 MiB` (backstop seguro sobre el máximo legítimo de UI: imágenes 5 MiB, proofs 10 MiB) + matcher de `contentType` (`isImage()` = `image/.*`; `isImageOrPdf()` para proofs y cotizaciones de proyecto). El matcher se aplica a **todas** las reglas de write incl. proofs (antes solo gated por ownership). **`delete` queda solo-ownership** (en un delete `request.resource` es null → el matcher error-denegaría; por eso `write` se parte en `create,update` + `delete`, preservando `removeBusinessGalleryPhoto`/`removeSchoolGalleryPhoto`). Subtileza clave (verificada con docs citadas): los recursive wildcards de Storage son **OR-aditivos sin most-specific-wins**, así que en vez de un catch-all `schools/{id}/**` se usan **tres bloques mutuamente excluyentes** (`{photo|cover}` image-only, `gallery/{file}` image-only, `projects/{pid}/**` image-**o**-pdf) — las cotizaciones PDF de proyecto solo se permiten en el subárbol de projects, y las imágenes de escuela siguen siendo image-only. Harness Storage **15→30 tests** (allow de cada writer legítimo + deny de stranger/oversize/wrong-type + PDF-quote allow + delete por owner); suite total **107 verdes** (77 firestore + 30 storage). → [`storage.rules`](../../storage.rules), [`test/rules/storage.rules.test.ts`](../../test/rules/storage.rules.test.ts). *(cierra #7)*
...
- ✅ **P1-f** · **Headers de seguridad** — IMPLEMENTADO (`next.config.ts` `headers()`, todo estático → **SSG intacto**, sin middleware/nonce). **Enforced** (seguros, no tocan carga de scripts ni orígenes Firebase/Maps): `Strict-Transport-Security` (2a, includeSubDomains, preload; solo prod), `X-Content-Type-Options: nosniff`, `Referrer-Policy: strict-origin-when-cross-origin`, `Permissions-Policy` (`geolocation=(self)` por los pickers de ubicación; resto bloqueado), `X-Frame-Options: DENY`, `Cross-Origin-Opener-Policy: same-origin-allow-popups` (popup-safe — `same-origin` rompería `signInWithPopup`), y una **CSP mínima enforced** (`frame-ancestors 'none'; base-uri 'self'; object-src 'none'; form-action 'self'` → clickjacking ya bloqueado). **CSP estricta completa en `Content-Security-Policy-Report-Only`** (rollout en observación, como pide el plan): allowlist verificado con citas (Firebase auth-popup/firestore/storage/functions, Maps `@vis.gl`, avatares Google) por directiva (script/connect/img/frame/style/font/worker-src); branch dev/prod (dev añade emuladores loopback + HMR `ws:` + `unsafe-eval`). `script-src` lleva `'unsafe-inline'` como puente para los inline de hidratación de Next + los ~9 JSON-LD hasta adoptar hash/nonce. **Flip-to-enforce** (documentado): observar 1-2 releases → promover directivas no-script (connect/img/font/style/frame/default) a enforced → decidir `script-src` (hash vía `experimental.sri` + componente JSON-LD compartido preserva SSG; nonce forzaría dinámico). Pendiente: wire de `Reporting-Endpoints`/`report-to` para agregación (hoy Report-Only solo emite a consola). Verificado: `npm run build` OK con `/`,`/categories`,`/schools` aún **Static**; headers confirmados con `curl -I` contra `next start`. → [`next.config.ts`](../../next.config.ts). *(cierra #9; CSP estricta en etapa Report-Only)*
...
- ✅ **P1-h** · **Confirm idempotente + guard de transición** — IMPLEMENTADO. El **único** efecto no-idempotente de los triggers era el append de auditoría (`auditEvents.add()` con id aleatorio); los recomputes ya son idempotentes (cuentan/suman desde la fuente y hacen `set`). Modelo de dedupe **dual**: (1) **`event.id` idempotency** — los triggers Gen2 son at-least-once, así que el audit pasó a `auditEvents.doc(sha256(event.id)).create(payload)` con catch de **solo** `ALREADY_EXISTS` (gRPC 6) **dentro** del recorder (si rechazara, `Promise.all` fallaría → tormenta de retries); una reentrega del mismo evento no-opera, una **renovación** legítima de suscripción es otro write con otro `event.id` → su propia fila (correcto). Se hashea `event.id` con sha256 porque el id crudo de Eventarc no es doc-id-safe (puede traer `/`). (2) **guard de transición en reglas** `confirmsContributionFromPending()` — una contribución se confirma **una sola vez** (`pending→confirmed`, sin renovación a diferencia de las suscripciones), así que un write que ponga `status:'confirmed'` solo se permite desde `pending`; bloquea que la escuela re-confirme una contribución ya confirmada (re-estampar `confirmedAt` → fila de auditoría duplicada bajo un `event.id` nuevo que el dedupe no atrapa). Las suscripciones **no** llevan guard de from-state (las renovaciones `confirmed→confirmed` son legítimas y deben auditar; su dedupe es solo `event.id`). (3) **robustez**: `confirmSubscription` ahora es transaccional (`runTransaction`) para cerrar el TOCTOU de `firstConfirmedAt` (dos confirmadores concurrentes). Verificado con citas (at-least-once, estabilidad de `event.id`, `create()`+catch-6 fuera de transacción) y red-team del diseño (truth table del guard: proof-upload/first-confirm/donor/admin/un-confirm no se rompen). Harness +3 (re-confirm contribución DENY, first-confirm ALLOW, renovación de suscripción ALLOW); suite **110 verdes**; functions compilan. → [`functions/src/index.ts`](../../functions/src/index.ts), [`firestore.rules`](../../firestore.rules), [`lib/firestore/subscriptions.ts`](../../lib/firestore/subscriptions.ts). *(cierra #11)*
...
- 🟢 **#13 (parcial)** · **`contact.email` reubicado a subcolección privada** — IMPLEMENTADO. El email del dueño se renderiza en **cero** páginas públicas pero vivía en el doc world-readable de `businesses` → email cosechable en cada scrape del catálogo. Movido a `businesses/{id}/private/data` (nuevo tipo `BusinessPrivate`), gated en reglas a **owner/editor/admin** (nunca anónimo; sin gate de verificación, a diferencia de las escuelas — un comercio no tiene ciclo de verificación ni lectura de soporte). El form de edición lee el email del subdoc privado con **fallback al legacy `contact.email`** (los docs pre-#13 lo prellenan y **migran al guardar**: el write público reescribe el mapa `contact` entero, scrubeando el legacy). El form de creación no captura email → sin el problema de batch-create del subdoc. Red-team confirmó que la remoción naive de los otros candidatos rompe SSR/round-trip (`subscription` alimenta `ManageBar`; `contact.catalog` se perdería por la preservación del form). Harness **+7** (owner/editor/admin read+write ALLOW; anónimo/stranger DENY). → [`types/firestore.ts`](../../types/firestore.ts), [`lib/firestore/businesses.ts`](../../lib/firestore/businesses.ts), [`firestore.rules`](../../firestore.rules), `app/(panel)/panel/business/[id]/edit/page.tsx`. **Backfill pendiente (ops):** script Admin-SDK que mueva a privado los emails legacy aún en docs públicos (hoy migran lazy en el próximo guardado del dueño). **Resto de #13 = ops:** App Check (P1-a) + CDN/WAF delante de GETs públicos + reubicar `metrics.*` (incl. el `walkIns` privado) que `trackInteraction`/`recordWalkIn` escriben **en el doc** — requiere cambio de Cloud Function, no de cliente.
...
- ✅ **#15** · **Validación de esquema de URL en UGC** — IMPLEMENTADO. Helper puro `safeExternalUrl()` ([`lib/url.ts`](../../lib/url.ts)): parsea con `new URL()` y devuelve el href **solo** si el protocolo parseado es `http:`/`https:`, si no `null` — allowlist del **protocolo** (no regex del string), que es lo que neutraliza el smuggling por mayúsculas / espacios / tab / newline / NUL dentro del esquema, `data:`/`vbscript:`/`blob:`/`file:`, y protocol-relative (`//evil`, que tira sin base). Aplicado en **render** (la frontera real — las reglas no pueden iterar elementos de una lista): `quoteUrls` de cada etapa en [`ProjectStageItem.tsx`](../../components/projects/ProjectStageItem.tsx) (página pública del proyecto) **y** en el editor del panel `projects/[pid]/page.tsx` (una cotización legacy con esquema inválido queda como texto inerte «(enlace inválido)» pero removible). Aplicado también en **write** (defensa en profundidad) vía `sanitizeStages()` en [`lib/firestore/projects.ts`](../../lib/firestore/projects.ts) (`createProject`/`updateProject`). `photos[]` no necesita el helper (`<img src>` no ejecuta `javascript:`, y van por `next/image` con `remotePatterns`). Red-team adversarial (Node WHATWG URL) confirmó cero bypass + descubrió el segundo sink del editor. Tests `lib/url.test.ts` (11) → **173 unit**; harness de reglas sin cambios (la frontera es código de render/write, no reglas). → [`lib/url.ts`](../../lib/url.ts), [`components/projects/ProjectStageItem.tsx`](../../components/projects/ProjectStageItem.tsx), [`lib/firestore/projects.ts`](../../lib/firestore/projects.ts). *(cierra #15)*
