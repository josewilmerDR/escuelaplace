 ...
 *  - `boxed`: a bordered card (the treatment the payment-methods / discount blocks had),
 *    for a self-contained sub-group that benefits from a visual frame.
 *
...
export function FormSection({
  ...
  return (
    <fieldset
      className={`${boxed ? "rounded-md border p-3" : ""} ${className}`.trim()}
    >
      <legend className={`text-sm font-medium ${boxed ? "px-1" : ""}`.trim()}>
        {legend}
      </legend>
      {description && (
        <p className={`text-xs text-muted ${boxed ? "mb-3 mt-1" : "mt-1"}`}>
          {description}
        </p>
      )}
      <div className={`${description || boxed ? "mt-3" : "mt-2"} ${contentClassName}`}>
        {children}
      </div>
    </fieldset>
  );
